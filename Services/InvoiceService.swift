...
class InvoiceService {
    ...
    func generatePDF(for invoice: Invoice, context: ModelContext) throws -> Data {
        ...
        let data = renderer.pdfData { context in
            ...
            // Draw Footer
            drawFooter(rendererContext: context, rect: pageRect, margin: margin, businessProfile: businessProfile)
        }
    ...
    private func drawFooter(rendererContext: UIGraphicsPDFRendererContext, rect: CGRect, margin: CGFloat, businessProfile: BusinessProfile?) {
        let smallFont = UIFont.systemFont(ofSize: 7)
        let businessName = businessProfile?.businessName ?? "FLO"
        let year = Calendar.current.component(.year, from: Date())

        // Growth loop: every invoice reaches another business owner — exactly
        // FLO's target customer. On by default; only Pro can turn it off in
        // Invoice Settings (tier enforced here so a downgrade re-enables it).
        let optedOut = UserDefaults.standard.object(forKey: "invoiceShowFLOBranding") as? Bool == false
        let showBranding = !(optedOut && SubscriptionManager.shared.currentTier == .pro)
        let footer = showBranding
            ? "©\(year) \(businessName) · Sent with FLO — finance & invoicing for freelancers · floptimizer.github.io/FLO"
            : "©\(year) \(businessName)"
        ...
        // Make the branding line tappable in PDF viewers
        if showBranding, let url = URL(string: "https://floptimizer.github.io/FLO/") {
            rendererContext.setURL(
                url,
                for: CGRect(origin: footerOrigin, size: footerSize)
            )
        }
