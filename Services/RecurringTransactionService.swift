...
//
//  Version 2.2 - Suggested Date Learning from Payment Logs
//  Copyright © 2026 Finch & Poppy Co LLC. All rights reserved.
//
//  CHANGES FROM v2.1:
//  ✅ Added suggestedNextDate(for:) that uses payment log history to predict best date
//  ✅ Added autoCreateFromLog integration — when markAsSent is used, engine skips that period
//
//  CHANGES FROM v2.0:
...
final class RecurringTransactionService {
    ...
    // MARK: - Suggested Date (v2.2)

    /// Returns the suggested next occurrence date for a recurring transaction,
    /// factoring in actual payment history from payment logs.
    /// Falls back to the standard nextOccurrence if insufficient payment history.
    func suggestedNextDate(for recurring: RecurringTransaction) -> (date: Date, isLearned: Bool)? {
        guard recurring.isActive else { return nil }

        // If we have a learned day from payment logs, use it
        if let suggestedDay = recurring.suggestedDayOfMonth,
           let standardNext = recurring.nextOccurrence {
            let calendar = Calendar.current
            var components = calendar.dateComponents([.year, .month], from: standardNext)
            components.day = suggestedDay
            ...
            if let firstOfMonth = calendar.date(from: DateComponents(year: components.year, month: components.month, day: 1)),
               let range = calendar.range(of: .day, in: .month, for: firstOfMonth) {
                components.day = min(suggestedDay, range.count)
            }

            if let learnedDate = calendar.date(from: components) {
                return (learnedDate, true)
            }
        }
    ...
    // MARK: - Duplicate Scrubber (v2.10)

    /// Result of a duplicate scrub pass.
    struct ScrubResult: Sendable {
        let duplicateGroups: Int
        let transactionsDeleted: Int
        let accountsRebalanced: Int
    }

    /// One-shot utility to remove duplicate recurring-materialized transactions caused by
    /// CloudKit sync races (two devices materializing the same recurring on the same day).
    /// Within each (recurringParent, calendar-day) group, the earliest-created transaction
    /// is kept and the rest are deleted. Each deleted transaction's amount is reversed on
    /// its linked Account so the stored balance matches reality.
    ///
    /// Safe to call multiple times — idempotent. But the caller should gate it behind a
    /// UserDefaults flag so it only runs once in normal operation.
    @discardableResult
    func scrubDuplicateRecurringTransactions(in container: ModelContainer) async -> ScrubResult {
        let context = ModelContext(container)

        // Fetch all transactions that were generated by a recurring parent.
        let descriptor = FetchDescriptor<Transaction>(
            predicate: #Predicate { $0.recurringParent != nil }
        )
        ...
        do {
            ...
            logger.error("❌ Scrubber failed to fetch recurring transactions: \(error.localizedDescription)")
            return ScrubResult(duplicateGroups: 0, transactionsDeleted: 0, accountsRebalanced: 0)
        }

        let calendar = Calendar.current

        // Group by (recurringParent identity, startOfDay(date)).
        // Using the RecurringTransaction's persistentModelID as the group key avoids
        // identity-compare quirks across SwiftData contexts.
        struct GroupKey: Hashable {
            let recurringID: PersistentIdentifier
            let day: Date
        }

        var groups: [GroupKey: [Transaction]] = [:]
        for tx in all {
            guard let parent = tx.recurringParent else { continue }
            let key = GroupKey(
                recurringID: parent.persistentModelID,
                day: calendar.startOfDay(for: tx.date)
            )
        ...
        var deleted = 0
        var skippedGroups = 0
        var rebalancedAccounts: Set<PersistentIdentifier> = []

        for (_, group) in groups where group.count > 1 {
            // v3.13 safety: only auto-scrub groups whose rows are materially
            // identical (same amount + category). If the user edited one copy
            // — changing its category, amount, etc. — the differences signal
            // intent, so we leave the group alone for manual resolution
            // rather than silently dropping edits. This matters because the
            // scrubber now runs every launch, not once per install.
            let firstAmount = group.first!.amount
            let firstCategoryID = group.first!.category?.persistentModelID
            let allIdentical = group.allSatisfy { tx in
                abs(tx.amount - firstAmount) < 0.01 &&
                tx.category?.persistentModelID == firstCategoryID
            }
            guard allIdentical else {
                skippedGroups += 1
                logger.info("⏭️ Scrubber: skipping edited duplicate group for \(group.first!.merchantName) on \(group.first!.date.formatted(date: .abbreviated, time: .omitted)) — user edits detected")
                continue
            }

            duplicateGroups += 1

            // Keep the earliest-created row; delete the rest.
            let sorted = group.sorted { $0.createdAt < $1.createdAt }
            let keeper = sorted.first!
            let victims = sorted.dropFirst()

            for victim in victims {
                // Reverse the victim's effect on the linked account balance.
                // Transaction amounts are always positive; isIncome determines direction.
                if let account = victim.account {
                    let signed = victim.isIncome ? victim.amount : -victim.amount
                    account.currentBalance -= signed
                    account.lastBalanceUpdate = Date()
                    account.touch()
                    rebalancedAccounts.insert(account.persistentModelID)
                }
            ...
            logger.notice("🧹 Scrubbed \(group.count - 1) duplicate(s) for \(keeper.merchantName) on \(keeper.date.formatted(date: .abbreviated, time: .omitted))")
        }

        if skippedGroups > 0 {
            logger.info("⏭️ Scrubber skipped \(skippedGroups) duplicate group(s) with user edits — resolve manually")
        }

        if deleted > 0 {
            do {
                try context.save()
                logger.notice("✅ Scrubber removed \(deleted) duplicate transaction(s) across \(duplicateGroups) group(s); rebalanced \(rebalancedAccounts.count) account(s)")
            } catch {
                logger.error("❌ Scrubber save failed: \(error.localizedDescription)")
            }
        } else {
            logger.info("ℹ️ Scrubber found no duplicates")
        }

        return ScrubResult(
            duplicateGroups: duplicateGroups,
            transactionsDeleted: deleted,
            accountsRebalanced: rebalancedAccounts.count
        )
    }
}
